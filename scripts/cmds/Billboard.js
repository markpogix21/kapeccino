...
module.exports = {
  ...
  onStart: async function ({ api, event, args, client, __GLOBAL, usersData, threadsData }) {
    ...
    let avatarBuffer = Buffer.from(getAvatar, 'utf-8');
    let baseImageBuffer = Buffer.from(getPorn, 'utf-8');

    let image = await loadImage(avatarBuffer);
    let baseImage = await loadImage(baseImageBuffer);
    ...
    return api.sendMessage({ attachment: fs.createReadStream(tempFilePath) }, threadID, () => fs.unlinkSync(tempFilePath), messageID);        
  }
};
